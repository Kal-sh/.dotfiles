...
const AppGestureSettingsRow = GObject.registerClass({
     ...
     */
    constructor(app, appGestureSettings, model) {
 ...
 */
const AppKeybindingGesturePrefsGroup = GObject.registerClass(class GIE_AppKeybindingGesturePrefsGroup extends Adw.PreferencesGroup {

    /**
     * @param prefsWindow parent preferences window
     * @param settings extension settings object
     */
    constructor(prefsWindow, settings) {
        super({
            title: 'Enable application specific gestures',
            description: 'Hold and then swipe horizontally to activate the gesture',
        });
 ...
 */
export function getAppKeybindingGesturePrefsPage(prefsWindow, settings, builder) {
    const page = new Adw.PreferencesPage({
        title: 'App Gestures',
