        ...
        const callback = () => {
            ...
            if (this._window) {
                ...
                if (this._direction === Clutter.Orientation.VERTICAL) {
                    ...
                    stSettings.slow_down_factor =
                        UPDATED_WINDOW_ANIMATION_TIME /
                ...
                // snap-left,normal,snap-right
                    ...
                    if (state !== GestureTileState.NORMAL) {
                        const keys = [
                            Clutter.KEY_Super_L,
                            state === GestureTileState.LEFT_TILE
                                ? Clutter.KEY_Left
                                : Clutter.KEY_Right,
                        ];
...
export class SnapWindowExtension {
    ...
    constructor(nfingerss) {
        this._connectors = [];
        this._toggledDirection = false;
        this._allowChangeDirection = false;
        this._swipeTracker = createSwipeTracker(global.stage, nfingerss, Shell.ActionMode.NORMAL, Clutter.Orientation.VERTICAL, true, 1, { allowTouch: false });
    ...
    _gestureBegin(tracker, monitor) {
        ...
        switch (progress) {
            ...
            case GestureMaxUnMaxState.MAXIMIZE:
                ...
                if (ExtSettings.ALLOW_FULLSCREEN_WINDOW &&
                    !window.is_monitor_sized() &&
                    !monitorArea.equal(window.get_buffer_rect()))
    ...
    _gestureUpdate(_tracker, progress) {
        ...
        else if (this._allowChangeDirection &&
            progress <= GestureMaxUnMaxState.UNMAXIMIZE - TRIGGER_THRESHOLD) {
