...
export class ForwardBackGestureExtension {
    ...
    constructor(appForwardBackKeyBinds, extensionPath, enableVerticalSwipe) {
        ...
        this._swipeTracker = createSwipeTracker(global.stage, [4, 3], //TODO: add support for vertical hold and swipe without disabling 3/4-fingers vertical swipe
        Shell.ActionMode.NORMAL, enableVerticalSwipe
            ? Clutter.Orientation.VERTICAL
            : Clutter.Orientation.HORIZONTAL, false, 1, { allowTouch: false });
    ...
    _gestureBegin(_tracker) {
        ...
        if (!this._focusWindow)
            return;
        this._animationState = AnimationState.WAITING;
        _tracker.confirmSwipe(global.screen_width, [AnimationState.LEFT, AnimationState.DEFAULT, AnimationState.RIGHT], AnimationState.DEFAULT, AnimationState.DEFAULT);
    }

    _gestureUpdate(_tracker, progress) {
        switch (this._animationState) {
            case AnimationState.WAITING:
                if (Math.abs(progress - AnimationState.DEFAULT) <
                    SnapPointThreshold)
                    return;
    ...
    _getClutterKeyForFocusedApp(gestureDirection) {
        ...
        if (keyBind) {
            ...
            const returnBackKey = (gestureDirection === SwipeGestureDirection.LeftToRight) !==
                keyBind[1];
